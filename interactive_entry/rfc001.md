...
| language     | string | Operate system language                      | No | vi reference: http://www.lingoes.cn/zh/translator/langcode.htm |
...
1. Build request url: 
    ...
    ```
    https://api.adfly.global/api/ig/entry/query?appKey=appkey&ci=urlencode(ci)&deviceId=urlencode(deviceId)&nonce=urlencode(nonce)&soltId=soltId&timestamp=timestamp&signature=signature
    ```
